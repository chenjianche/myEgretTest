module fighter
{
    ...
    export class GameContainer extends egret.DisplayObjectContainer
    {
        ...
        /**我的血条 */
        private myBloodBar:egret.Bitmap;
        private myBloodBarWidth:number=150;

        /**myBuff */
        private myBuffs:fighter.Buff[]=[];

        /**myPower */
        private myPowerLevel:number=0;
        ...
        private createGameScene():void{
            ...
            //创建血条
            var hpImg:egret.Bitmap = fighter.createBitmapByName("hp_png");
            hpImg.x = 10;
            hpImg.y = 10;
            this.addChild(hpImg);
            var bloodBarBg:egret.Bitmap = fighter.createBitmapByName("bloodBarBg_png");
            bloodBarBg.x = 80;
            bloodBarBg.y = 15;
            bloodBarBg.width = this.myBloodBarWidth;
            this.addChild(bloodBarBg);

            this.myBloodBar = fighter.createBitmapByName("bloodBar_png");
            this.myBloodBar.x = 80;
            this.myBloodBar.y = 15;
            this.myBloodBar.width=this.myBloodBarWidth;
            this.addChild(this.myBloodBar);
        ...
        private preCreatedInstance():void {
            ...
            for(i=0;i<20;i++){
                var buff:fighter.Buff = fighter.Buff.produce("buff_blood",BuffTypes.BloodAdd);
                objArr.push(buff);
            }
            for(i=0;i<20;i++) {
                buff = objArr.pop();
        ...
        private createBulletHandler(evt:egret.Event):void{
            ...
            if(evt.target==this.myFighter) {
                    ...
                    if(this.myPowerLevel==0 || this.myPowerLevel==2){
                        bullet = fighter.Bullet.produce("b1");
                        bullet.x =this.myFighter.x + (this.myFighter.width-29)/2;
                        bullet.y = this.myFighter.y+30;
                        this.addChildAt(bullet,this.numChildren-1-this.enemyFighters.length);
                        this.myBullets.push(bullet);
                    }

                    if(this.myPowerLevel>=1) {
                        for(var i:number=0;i<2;i++) {
                            bullet = fighter.Bullet.produce("b1");
                            bullet.x = i==0?(this.myFighter.x):(this.myFighter.x+this.myFighter.width-22);
                            bullet.y = this.myFighter.y+30;
                            this.addChildAt(bullet,this.numChildren-1-this.enemyFighters.length);
                            this.myBullets.push(bullet);
                        }
        ...
        /**创建buff */
        private createBuff(x:number,y:number){
           var rr = Math.random() *100;
           //egret.log(rr);
           //10%的几率出加血
           var buffType:BuffTypes;
           var textureName:string;
           if(rr>=95){
               buffType= BuffTypes.BloodAdd;
               textureName = "buff_blood";
           }else if(rr<=5){
               buffType = BuffTypes.PowerUp;
               textureName = "buff_power";
           }else{
               return;
           }
           var buff:fighter.Buff = fighter.Buff.produce(textureName,buffType);
            buff.x = x;
            buff.y = y;
            this.addChildAt(buff,this.numChildren-1);
            this.myBuffs.push(buff);
        }
        /**游戏画面更新*/
        private gameViewUpdate(evt:egret.Event):void{
           ...
           for(i = 0;i < enemyBulletsCount;i++){
            ...
            //buff移动
            var buffCount:number = this.myBuffs.length;
            var buff:fighter.Buff;

            for(i=0;i<buffCount;i++){
                buff = this.myBuffs[i];
                if(buff.y>this.stage.stageHeight){
                    this.removeChild(buff);
                    Buff.reclaim(buff);
                    this.myBuffs.splice(i,1);
                    i--;
                    buffCount--;
                }
                buff.y += 3 * speedOffset;
            }
            this.gameHitTest();
        }
        /**设置我的血条 */
        private setMyBloodBar(blood:number){
            if(blood<=10){
                var m = this.myBloodBarWidth /10;
                this.myBloodBar.width = m*blood;
            }
        }
        ...
        private gameHitTest():void {
            ...
            for(i=0;i<enemyBulletsCount;i++) {
                ...
                if(fighter.GameUtil.hitTest(this.myFighter,bullet)) {
                    ...
                    this.setMyBloodBar(this.myFighter.blood);
                    if(delBullets.indexOf(bullet)==-1)
                        delBullets.push(bullet);
                }
            }
            ...
            if(this.myFighter.blood<=0) {
                ...
                while(delFighters.length>0) {
                    ...
                    fighter.Airplane.reclaim(theFighter);
                        ...
                        //消灭敌机尝试生成buff
                        this.createBuff(theFighter.x,theFighter.y);
                }
            }

            //碰buff
            var buffCount = this.myBuffs.length;
            var buff:fighter.Buff;
            for(i=0;i<buffCount;i++){
                buff = this.myBuffs[i];
                if(fighter.GameUtil.hitTest(this.myFighter,buff)){
                    if(buff.BuffType == BuffTypes.BloodAdd){
                        if(this.myFighter.blood<10){
                            this.myFighter.blood +=1;
                            this.setMyBloodBar(this.myFighter.blood);
                        }
                    }else{
                        if(this.myPowerLevel<2){
                            this.myPowerLevel++;
                        }
                    }
                    this.removeChild(buff);
        ...
        private gameStop():void{
            ...
            //清理buff
            var buff:fighter.Buff;
            while(this.myBuffs.length>0){
                buff = this.myBuffs.pop();
                this.removeChild(buff);
                fighter.Buff.reclaim(buff);
            }
            this.myPowerLevel=0;
            this.setMyBloodBar(10);
            //显示成绩
